                       ...
                       std::vector<float> &outResults)
{
  ...
  // else
  // {
  //   unsigned int sequenceLenght = 32;

  //   for (unsigned int i = 0; i < inputData.size() - sequenceLenght + 1; i++)
  //   {
  //     // Copy input data (only sequence) to tensor
  //     std::copy(inputData.begin() + i, inputData.begin() + i + sequenceLenght, input_buffer);

  //     m_interpreter->Invoke();

  //     outResults[i] = *output_buffer;
  //   }
  // }
